    ...
    [ interval ] : 간격 뜻,
        ...
        2. 사용법 
            (1) setInterval(함수명, 밀리초)
                - 함수명 : 함수명 만 작성한다. () 생략하여 실행
                - 밀리초 : 1/1000초 , 1000:1초, 10000:10초, 100:0.1초
                    -> 반환값(함수 실행 후 돌려주는/반환/결과)
                    let 변수 = setInterval();

            (2) clearInterval(interval 객체);
                - 중지 할 interval 객체를 대입하여 해당 interval 중지하는 함수 : 
*/

//[1] 예제1
let value=0; //전역변수
function 증가함수(){ //value 1증가 함수
    //1. 전역변수 1증가
    value=value +1 // vs value += 1 va value ++
    //2. 특정한 위치에 전역변수 값 대입
        // - 어디에
...
// ** 간격 (1초) 에 따른 '증가 함수' 재실행 **
setInterval(증가함수,1000); // 1000밀리초(1초) 간격으로 '증가함수'
...
function 타이머시작(){
    ...
    // 전역변수 사용 이유 : 서로 다른 함수에서 공통변수를 사용이 가능하다.
    // vs 지역변수 : 함수 안에서만 사용한다.
    timerId=setInterval(시간증가,1); //1초마다 '시간증가' 함수를 실행
}
