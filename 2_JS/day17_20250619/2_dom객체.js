...
//   const h1 = document.querySelector('#title').innerHTML='DOM조작성공!';
    
    h1.innerHTML ='DOM조작성공!'
...
const item=document.querySelectorAll('.item');
    for(i=0; i<=item.length -1; i++){
        const coloritem=item[i]
        coloritem.style='color:green; font-weight:bold;'
    }
...
function func9(){
    ...
    //4. a마크업 객체내 value 속성값을 가져온다.
    const value =itemInput.value
    //5. value(입력받은값)를 (전역)배열에 추가한다.
    arr.push(value);
    //6. (출력)'arrayStatus' 마크업 객체를 가져온디/
    ...
    for(i=0;i<=arr.length-1; i++){
        ...
        html +=`<span> ${array}, </span>`//html 구성

    }
    //8. 배열 정보를 HTML 구성한 문자열을 innerHTML 대입
