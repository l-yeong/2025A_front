...
// * 배열내 모든 정수 조회 0부터 마지막 인덱스 까지 조회

// let scores = [85, 92, 78, 65, 95];
// let sum = 0; //누적 총 합계를 구해야 하므로 for 밖에 선언
...
//     blood= bloodTypes[index]; // index 번째의 혈액형 호출
//     if(blood=='A'){
//         countA++; //vs countA +=1 vs countA =  countA+1 3개중 다 똑같음
//     }
// }
// console.log(`A형 : ${countA}`)
...
//     output += `<div> ${name} </div>`;// HTML 문자열 추가 </div> 지우면 동그라미가 이름 옆으로감 93줄 주석 지우기
//     //점수출력
//     let score = scores[index]; 
//     let black = parseInt(score/10); // 92/10 -> 9.2 -> parseInt(9,2) ->9
//     for(let circle=1; circle <= 10; circle++){
//         //동그라미는 1부터 10이하 까지 1씩 증가 (총 10번)
//         if(circle <= black){
//             output += `<span>●</span>`
//         }else{ //크면 흰색
//             output += `<span>○</span>`
