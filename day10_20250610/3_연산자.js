...
/* 
[지문1 내가 한것]
let 국어=Number(prompt("국어 점수 입력 하세요"));
let 수학=Number(prompt("수학 점수 입력 하세요"));
let 영어=Number(prompt("영어 점수 입력 하세요"));
...
*/
// 4. 논리연산자 : 하나의 연산결과를 (논리) 값으로 반환, && / || 항상 2개 이상의 비교 연산자 필요하다
/*
    && 이면서 면서 이고 그리고 모두 and 조건, 조건이 모두 true 이면 true
    || 이거나 거나 하나라도 또는 or 조건, 조건이 하나라도 true 이면 true
    ! 부정 반대 not 조건, true -> false, false->true
        초등학생이 사탕이면서 초콜릿 먹는다. and 조건, 2개다 먹어야 true (충족)
        초등학생이 사탕이거나 초콜릿 먹는다. or 조건, 1개이상 먹으면 true (충족)
        초등학생이 사탕 아니고, not조건, 사탕이 아닌
        * 범위논리 지원하지 않는다. 5 < 점수 < 10 (파이썬만됌)
        점수 > 5 && <10 조건으로 해야됌
...
// [지문4] prompt 함수로 정수를 입력받아 입력받은 값이 홀수이면 true / 짝수이면 false 로 console탭에 출력하시오.
    // 홀/짝 계산식 : 값이 나누기 2를 했을때 나머지가 1이면 홀수 이고 0이면 짝수, 값 % 2 == 1, 값 % 2== 0
...
// [지문5] prompt 함수로 정수를 입력받아 입력받은 값이 7의 배수이면 true / 아니면 false 로 console탭에 출력하시오.
    //배수 계산식 : 값이 나누기 수를 했을때 나머지가 0이면 해당 값은 그 수의 배수값, 값 % 수 == 0
...
/*
let 아이디=prompt("아이디 입력하세요");
let 비밀번호=Number(prompt("비밀번호 입력하세요"));
...
/*
let 정수=Number(prompt("정수 입력하세요"))
결과=(정수%2==1)&&(정수%7==0)
console.log(`결과: ${결과}`)
*/


//5. 할당연산자/(복합)/대입연산자 : 변수 저장할때 사용되는 기호
/*
...
// 6. 증(가)감(소)연삼자, <반복문>
// 변수명 앞뒤로 ++ : 1씩 증가 , -- : 1씩감소
// ++변수명 : 선위증가, 변수명++ : 후위증가 후위란 ;(세미클론) 기준으로 명령어에서 가장 나중에 처리
// --변수명 : 선위감소, 변수명-- : 호위감소
let 나이=40;
...
console.log(나이); 42
--나이; // 증강연산자와 외 다른 명령어가 없으면 선위/후위 가 상관없다. * 세미클론 단위 우선순위 따짐.
나이--;

// 7. **삼항연산자**, 항이 3개, (1)조건 ? (2)참일때 : (3)거짓일때
// (단일 조건) 조건 ? 참 : 거짓
// (중첩 조건)조건1 참1 : 조건2 ? 참2 : 조건
...
/*
let 일차점수=Number(prompt("1차점수:"))
let 이차점수=Number(prompt("2차점수:"))

let 결과 =일차점수+이차점수 >=150 ?  "합격" : "불합격";
console.log(`결과: ${결과}`)
*/
...
let 이름=prompt("이름:");
let result= 이름 == "유재석" ? `${이름}방장` : `${이름}` //논리는 항상 true 와 false 로 답한다.
//           (논리)조건            참           거짓
