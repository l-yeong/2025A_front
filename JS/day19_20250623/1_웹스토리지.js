/*
    [ 웹스토리지 ]
        - 웹브라우저(개발자도구) -> [application]탭 -> 왼쪽사이드 메뉴 (local/session)
        - 도메인 마다 웹스토리지가 할당된다
        1. 종류
            (1)sessionStorage   : 브라우저가 실행 동안만 저장, 브라우저를 닫으면 (자동)데이터 사라진다.
                - 일회성 정보, 이전페이지 기록
            (2)localStorage     : 사용자가 직접 삭제 하거나 브라우저(캐시) 청소 하지 않는 한 영구 저장한다.
                - 설정 정보, 팝업창(다시보지않기), 테마, 자동로그인 등등 
        2. 사용법
            sessionStorage 객체
            (1) sessionStorage.setItem('속성명/key', 값/value);   : 저장
            (2) sessionStorage.getItem('속성명/key');             : 불러오기
            (3) sessionStorage.removeItem(속성명/key);            : 삭제
            (4) sessionStorage.clear();                          : 전체삭제
            * 속성명/key는 무조건 문자만 가능
            * value는 객체와배열 타입 저장 불가능. 리터럴 자료만 가능
    [ JSON ] : 자바스크립트객체 문법의 형식, JavaScript Object Notation
        * 자바객체를 문자타입 이지만 객체형식/모양으로 표현한것
        - 3 : 숫자타입, 숫자모양                vs "3" :문자열타입, 숫자모양
        - { age : 40 } : 객체타입, 객체모양     vs "{age:40}" :문자열 타입,객체모양
        - 왜??? 인터넷상의 데이터통신하는 http는 객체지향이 아니다. 문자열과 바이트 전송 가능
...
// [1] 웹스토리지 제어하는 객체
console.log(sessionStorage);    // 현재 웹스토리지 저장된 자료 갖는 객체 반환
console.log(localStorage);      // 현재 웹스토리지 저장된 자료 갖는 객체 반환
...
console.log(sessionStorage);        // 유재석이 없음 콘솔 34줄 봐야함
localStorage.clear();               // 전체 삭제
console.log(localStorage)           // 40 없어짐 콘솔 36줄 봐야함


// [5] 웹스토리지에 JSON 객체 저장
// sessionStorage.setItem('m1',['유재석',40]); //[X] 대활호 포함 하지 않음
sessionStorage.setItem('m1',JSON.stringify(["유재석",40])); //[O] 대괄호 포함 저장
